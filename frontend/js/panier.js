...
for (let m = 0; m < nameStorage.length; m++) {
  let priceProductBasket =
    (nameStorage[m].priceProduct / 100) * nameStorage[m].quantity;
...
const priceHtml = `  
  <div class=""> Total de la commande: ${new Intl.NumberFormat(
    "fr-FR",{ style: "currency", currency: "EUR" }).format(totalPriceCalcul)} </div>
`;
totalInner.innerHTML = priceHtml;
    ...
    const emptybasket = ` 
            <div class="container">
              <div> Votre panier est vide </div>
                            ...
                            `;
    panierPosition.innerHTML = emptybasket;
    const hideForm = (document.getElementById("#form").style.display =
      d - none);
...
address.addEventListener("change", (event) => {
  ...
  disable();
  message(
    "L'adresse ne doit contenir que des lettres et / ou chiffres sans ponctuation. ",
    addressOk
  );
});
